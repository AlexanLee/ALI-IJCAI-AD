...
def DefaultValidation(X, y, clf, lossfunction):
    ...
    for train_index, test_index in kf.split(self.df):
        X_train, X_test = self.df.ix[train_index, :][self.features], self.df.ix[test_index, :][
            self.features]
...
class LRS_SA_RGSS_combination():
    def __init__(self, clf, df, RecordFolder, columnname, start, label, Process, direction,
                 LossFunction, validatefunction=0, Startcol=['None'], PotentialAdd=[],
                 CrossMethod=0, CoherenceThreshold=1):
        self.clf = clf
    ...
    def select(self):
        ...
        while self.evaluate(self.greedyscore, self.score) | self.ScoreUpdate():
            ...
            print('random select starts with:\n {0}\n score: {1}'.format(self.bestfeature,
                                                                         self.greedyscore))
            # random selection
            if self.Process[1]:
                self.MyRandom()
            ...
            self.ScoreUpdate()
        print(
        '{0}\nbest score:{1}\nbest features combination: {2}'.format('*-*' * 50, self.bestscore,
                                                                     self.bestfeature))

    def validation(self, selectcol, num, addfeature,
        ...
        if self.evaluate(np.mean(totaltest), self.score):
            ...
            if self.coherenceThreshold != 1:  # execute in the features adding process
                ...
                cc = [pearsonr(self.df[addfeature], self.df[ct])[0] for ct in
            ...
            if (np.abs(np.max(cc)) < self.coherenceThreshold):
                with open(self.RecordFolder, 'a') as f:  # record all the imporved combination
                    f.write('{0}  {1}  {2}:\n{3}\t{4}\n'.format(num, addfeature, np.abs(np.max(cc)),
    ...
    def Greedy(self):
        ...
        self.dele = ''
        self.bestscore, self.bestfeature = self.score, self.TemplUsedFeatures[:]
        while (self.Startcol != self.TemplUsedFeatures) | (
            self.PotentialAdd != []):  # stop when no improve for the last round and no potential add feature
            ...
            for sr, i in enumerate(self.TemplUsedFeatures[
...
class Select():
    ...
    def run(self, validate):
        ...
        a = LRS_SA_RGSS_combination(df=self.df,
                                    ...
                                    PotentialAdd=self.PotentialAdd,
                                    # potential feature for Simulated Annealing
